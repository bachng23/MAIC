...
@pytest.mark.anyio
async def test_get_drug_info_aggregates_direct_sources(monkeypatch) -> None:
    async def _fake_call_openfda(_drug_name: str) -> dict:
        return {
            ...
            "geriatric_use": "Monitor bleeding risk in elderly patients.",
    ...
    assert "Warfarin may increase bleeding risk." in result.interactions
    assert "Taiwan FDA ingredients: acetylsalicylic acid" in result.interactions
